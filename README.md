...
## Modification Notes (jmelms)
1. Modified DL_weather_dynamics.yml: python -> python=3.10 to ensure a compatible version of onnxruntime-gpu would be available, since it's not with python=3.14 as of 2026/02/02.
2. Need to run this on BigRed200 rather than Quartz as of 2026/02/03 because onnxruntime-gpu requires cuda>=12.8 and cudnn>=9, which are on the former machine but not the latter.
3. 